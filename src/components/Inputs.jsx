export function DisabledInput({ children, styles, answer }) {
  return (
    <div className="mt-3 flex flex-col">
      ...
      <input
        className={`${styles} bg-black-200 w-60 font-inter font-medium text-xs w-3/4 h-10 rounded-sm p-1 border-2 border-dark-gray`}
        type="text"
        name="name"
        placeholder={answer}
        disabled
...
export function InputDescription({ children, styles, answer, ...props }) {
  return (
    <div className="mt-3 flex flex-col">
      ...
      <input
        className={`${styles} w-60 font-inter font-medium text-xs w-3/4 h-10 rounded-sm p-1 focus:outline-none focus:border-sky-500 border-2 border-dark-gray`}
        type="text"
        name="name"
        placeholder={answer}
        {...props}
